...
# Questão 1 - OK
def questao_1(users):
    "Questão 1) Registe as informações dos seguintes usuários e realize a consutla para apresentar os dados."
    users_list = []
    for user in users:
        ...
        user_inserted = redis_conn.hgetall(f"user:{user["id"]}")
        user_inserted["id"] = user["id"]
        users_list.append(user_inserted)
...
def questao_2(interests):
    "Questão 2) Registre uma lista de interesses para cada um dos usuários e realize a consutla para apresentar os dados."
    users_list = []
    ...
    for user in interests:
        ...
        for interest in user['interesses']:
            for key, value in interest.items():
                result = redis_conn.zadd(f"user:{user_id}:interest", {key: value})
                print(result)
        
        interests = redis_conn.zrange(f"user:{user_id}:interest", 0, -1, withscores=True)
        users_list.append(interests)
...
# Questão 3 - OK
def questao_3(posts):
    "Questão 3) Resgistre as informações sobre os posts mais recentes e realize a consutla para apresentar os dados."
    posts_list = []
    for post in posts:
...
def questao_4(user_id):
    """
    Questão 4)
    Imagine que o usuário 3 acessa o seu feed. Realize uma consulta nos dados cadatrados para mostrar a lista dos posts mais interessantes para esse usuário.
    """
